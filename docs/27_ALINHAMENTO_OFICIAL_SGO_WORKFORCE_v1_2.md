...
**Status:** Alinhamento oficial vigente e suficiente para iniciar o Incremento 1  
**Finalidade:** consolidar o entendimento correto do produto, definir o primeiro incremento técnico, registrar a decisão provisória de modelagem das pausas e separar decisões bloqueadoras de decisões futuras.  
...
# 15. Matriz oficial de suficiência e decisões por fase

## 15.1 Conclusão de suficiência

O entendimento atual é **suficiente para iniciar o Incremento 1**.

Não existe decisão adicional obrigatória por parte do responsável pelo produto para implementar e validar:

- Jornada;
- Atividade;
- Pausa vinculada à atividade;
- máquina de estados;
- bloqueio de sobreposição;
- cálculo de duração bruta;
- cálculo de duração líquida;
...
As decisões ainda abertas pertencem a incrementos posteriores. Essas decisões não devem impedir o desenho técnico nem a implementação do motor de domínio.

## 15.2 Decisões fechadas para o Incremento 1

Para o primeiro incremento, ficam oficialmente definidos:

1. O Workforce permanecerá separado do SGO.
2. O primeiro incremento será exclusivamente de domínio, sem interface ou infraestrutura.
3. O HH oficial será calculado por timestamps persistidos.
4. A Jornada terá os estados `NAO_INICIADA`, `ABERTA` e `ENCERRADA`.
5. A Atividade terá os estados `CRIADA`, `ATIVA`, `PAUSADA` e `ENCERRADA`.
6. A Pausa terá os estados `CRIADA`, `ATIVA` e `ENCERRADA`.
7. A pausa será um evento próprio vinculado à atividade principal.
8. A atividade manterá o intervalo bruto entre início e fim.
9. A pausa terá timestamps próprios.
10. A duração da pausa será integralmente descontada da atividade no teste inicial.
11. O motivo provisório será `PAUSA_TESTE`.
12. Pausa sem atividade principal não será permitida.
13. Não será permitido iniciar outra atividade durante uma pausa.
14. Não será permitido encerrar atividade ou jornada com pausa aberta.
15. Apenas uma jornada poderá estar aberta por colaborador.
16. Apenas uma atividade principal poderá estar ativa.
17. Apenas uma pausa poderá estar ativa.
18. Sobreposições incompatíveis serão bloqueadas.
19. Timestamps finais não poderão ser anteriores aos iniciais.
20. Edição posterior e encerramento forçado não serão implementados neste incremento.
21. Múltiplas OS, rateio, GPS, RASF, offline, banco e dashboards estão fora do Incremento 1.

## 15.3 Decisões que não bloqueiam o Incremento 1

As seguintes definições permanecerão abertas até a fase correspondente:

### Antes do Incremento 2: persistência local

- política de retenção local após gravação;
- formato definitivo de serialização;
- recuperação de estado após fechamento;
- comportamento perante corrupção da base local.

### Antes do Incremento 3: sincronização

- convenção final de UUID do cliente;
- política de retry e backoff;
- tamanho máximo de lote;
- regras de conflito;
- retenção local após confirmação do servidor;
- autenticação técnica da API.

### Antes do Incremento 4: interface de campo

- identidade visual inicial;
- sequência exata dos botões;
- mensagens operacionais;
- confirmação de encerramento;
- dispositivo e navegador de referência do piloto.

### Antes do Incremento 5: catálogo operacional

- catálogo oficial de pausas;
- classificação produtiva, improdutiva ou não computável;
- regra de cômputo de cada motivo;
- pausa sem atividade principal;
- deslocamentos, esperas, DDS, reunião, treinamento e apoio;
- aprovação e vigência das alterações de catálogo.

### Antes do Incremento 6: atendimento de falhas

- catálogo oficial do MVP derivado do RASF;
- regra de atualização periódica do RASF;
- evidência fotográfica obrigatória ou opcional;
- obrigatoriedade de OS relacionada;
- tratamento de falha sem ativo localizado;
- campos adicionais de sistema, componente, impacto e pendência;
- fluxo de aprovação de novo sintoma, causa ou ação.

### Antes do Incremento 7: GPS

- intervalo padrão de pulsos;
- política adaptativa por movimento e bateria;
- obrigatoriedade de GPS;
- contingência por indisponibilidade ou baixa precisão;
- precisão mínima desejável;
- retenção dos dados;
- perfis autorizados a visualizar trajetórias;
- validações de LGPD, segurança da informação e política corporativa.

### Antes do Incremento 9: dashboards

- indicadores obrigatórios por perfil;
- filtros padrão;
- metas e referências;
- limites de detalhamento individual;
- periodicidade de atualização.

### Antes do Incremento 11: exportações

- layout oficial de CSV e XLSX;
- dados pessoais permitidos;
- política de auditoria da exportação;
- perfis autorizados;
- requisitos para GeoJSON.

### Antes do Incremento 12: capacidade PCM

- fonte oficial de escala;
- fonte de ausências e férias;
- definição de capacidade bruta;
- buckets oficiais de perdas;
- horizonte de projeção;
- tratamento de sazonalidade;
- regras do simulador e premissas editáveis.

### Antes do Incremento 13: integração com SGO

- contrato de usuários;
- contrato de ativos e pátios;
- contrato de OS e ciclo de programação;
- regra de devolução do HH real;
...
# 16. Perguntas ao responsável pelo produto e momento correto de resposta

## 16.1 Nenhuma resposta adicional é necessária para iniciar o Incremento 1

O agente não deverá interromper o início do desenho técnico para solicitar definições de GPS, RASF, múltiplas OS, rateio, catálogos, edição posterior, dashboards ou integração.

Essas perguntas deverão ser apresentadas somente quando a fase correspondente se aproximar.

## 16.2 Definições úteis, mas não bloqueadoras, que poderão ser antecipadas

O responsável pelo produto poderá, quando conveniente, adiantar:

1. Dois ou três motivos reais de pausa para o futuro piloto.
2. Qual aparelho e navegador serão usados no teste de campo.
3. Quais coordenações ou equipe formarão o grupo piloto.
4. Qual perfil será o primeiro consumidor do dashboard.
5. Se o piloto usará usuários fictícios ou matrículas reais.
6. Qual período do RASF será usado para validar os catálogos.
7. Qual arquivo ou fonte será referência da base de ativos.

Essas informações ajudarão o planejamento, mas não deverão entrar no código do Incremento 1.

## 16.3 Regra de governança de decisão

Toda decisão futura que altere regra de negócio, integridade, privacidade, sincronização, capacidade ou integração deverá ser registrada em ADR contendo:
...
# 17. Diretriz de ação do agente após esta versão

Após ler este documento, o agente deverá:

1. Declarar que o entendimento é suficiente para iniciar o Incremento 1.
2. Não repetir perguntas pertencentes a fases futuras como condição para começar.
3. Apresentar o desenho técnico do motor de domínio antes de escrever código.
4. Separar a implementação em micro-sessões.
5. Aguardar aprovação do desenho.
6. Implementar apenas a micro-sessão autorizada.
7. Criar testes junto com cada regra de domínio.
8. Manter fora do código qualquer premissa ainda não validada.

## Resposta esperada do agente

```text
O entendimento é suficiente para iniciar o Incremento 1.

Não há decisão adicional bloqueadora para Jornada, Atividade, Pausa e HH.

As pendências de GPS, RASF, catálogos, múltiplas OS, rateio, edição, dashboards e integração serão tratadas nas fases correspondentes.

O próximo passo é apresentar o desenho técnico do motor de domínio, sem criar arquivos, e aguardar aprovação.
```

---

# 18. Prevalência documental

Esta versão 1.2 é o alinhamento oficial vigente.

Em caso de divergência sobre:

- escopo do primeiro incremento;
- modelagem provisória da pausa;
- decisões bloqueadoras;
- ordem de construção;
- separação entre SGO e Workforce;

prevalecerá este documento:

```text
docs/27_ALINHAMENTO_OFICIAL_SGO_WORKFORCE_v1_2.md
```

Versões anteriores devem permanecer apenas em histórico e não devem ser utilizadas como referência ativa de implementação.
